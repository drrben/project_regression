...
ggpairs(new_abalone_train, title="Pairs plot for abalone dataset") + theme_grey(base_size = 8)
```

```{r}
new_abalone_scale = rapply(new_abalone_train, scale, c("numeric","integer"), how="replace")

linear_mod_log = lm(log_rings ~ Height + Whole_wt + Length, data=new_abalone_scale)
...
linear_mod_log = lm(log_rings ~ Height + Shuck_wt + Visc_wt + Shell_wt + Length, data=new_abalone_scale)
summary(linear_mod_log)
plot(linear_mod_log)
durbinWatsonTest(linear_mod_log, max.lag=10)
acf(resid(linear_mod_log))
...
#PUT HERE AGAIN THE SUMMARY OF OUR LAST MODEL OF WE JUST REFER TO PREVIOUS COMPUTATIONS ? 
summary(linear_mod_log)
```
