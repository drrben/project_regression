...
```{r}
linear_mod_log = lm(log_rings ~ poly(Diameter, 3) + poly(Height, 3) + poly(Shuck_wt, 3) + poly(Shell_wt, 3), data=new_abalone_scale)
summary(linear_mod_log)
plot(linear_mod_log)
durbinWatsonTest(linear_mod_log, max.lag=10)
acf(resid(linear_mod_log))
...
#Q10
```
To check which variables have a significant impact on the target variable (number of rings), we look at the p-values of the t-tests. If the p-value of the single variable is low it means that at a specific significance level the variable has a significant impact on the number of rings. Some of the covariates are not significant so we performed test with other models to obtain an high R adjusted but also low p-values for the t-test. 
...
linear_mod_log = lm(log_rings ~ Height + Shell_wt + Shell_wt2 + Shell_wt3 + Shuck_wt + Shuck_wt2 + Diameter2 + Diameter3, data=new_abalone_scale)
summary(linear_mod_log)
plot(linear_mod_log)
durbinWatsonTest(linear_mod_log, max.lag=10)
acf(resid(linear_mod_log))
...
Through the use of polynomial features we slightly improved all the postulates. The errors are centered, normal, and less correlated than before and the model is homoschedastic.
```{r}
#Q11
linear_mod_log_1 = lm(log_rings ~ 1, data=new_abalone_scale)
anova(linear_mod_log_1, linear_mod_log)
```
We implemented the initial linear model on the scaled data. The model explained variance of our model is MSS= 1682.1
```{r}
linear_mod_log_simple = lm(log_rings ~ Height, data=new_abalone_scale)
anova(linear_mod_log, linear_mod_log_simple)
```
we can see that it has a really small p-value and so the removal of the other other covariates results in a worse model. We are testing that all the variables are not correlated with log_rings besides Height. Since the p-value is very low we reject that all the variables are incorrelated and increasing the number of covariates improved our model. 

We can also prove that our last model is better using cross-validation
```{r}
library(DAAG) #TO PUT THE LIBRARY AT THE BEGINNING
cv.lm(new_abalone_scale,log_rings ~ Height + Shell_wt + Shell_wt2 + Shell_wt3 + Shuck_wt + 
    Shuck_wt2 + Diameter2 + Diameter3)
```
```{r}
cv.lm(new_abalone_scale,log_rings ~ Height)
```
```{r}
#Q12 
